-- Adicionar subcategoria "Reserva de emergência" nas categorias de investimento
-- Primeiro, buscar o ID da categoria pai "Investimentos e Aportes" 
-- e adicionar a nova subcategoria

DO $$
DECLARE
  v_investment_parent_id uuid;
BEGIN
  -- Buscar ID da categoria pai de investimentos (pode variar por usuário)
  -- Vamos adicionar para todos os usuários que já têm categorias
  
  FOR v_investment_parent_id IN 
    SELECT id FROM public.categories 
    WHERE type = 'expense' 
    AND name = 'Investimentos e Aportes'
  LOOP
    -- Verificar se já existe antes de inserir
    IF NOT EXISTS (
      SELECT 1 FROM public.categories 
      WHERE parent_id = v_investment_parent_id 
      AND name = 'Reserva de Emergência'
    ) THEN
      INSERT INTO public.categories (user_id, name, type, is_essential, color, parent_id)
      SELECT user_id, 'Reserva de Emergência', 'expense', true, '#10b981', v_investment_parent_id
      FROM public.categories
      WHERE id = v_investment_parent_id;
    END IF;
  END LOOP;
END $$;

-- Atualizar a função create_default_categories para incluir a nova subcategoria
...
BEGIN
  ...
  (p_user_id, 'Reserva de Emergência', 'expense', true, '#10b981', v_financeiro);
  ...
  (p_user_id, 'Reserva de emergência', 'investment', '#22c55e'),
