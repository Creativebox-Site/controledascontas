...
-- Create audit table for tracking access to sensitive personal data
...
-- Only service role can read/write audit logs (prevents tampering)
...
CREATE INDEX idx_profile_access_audit_user_id ON public.profile_access_audit(user_id);
CREATE INDEX idx_profile_access_audit_accessed_at ON public.profile_access_audit(accessed_at DESC);

-- ===== AUDIT TRIGGER FOR PROFILES TABLE =====
-- Function to log profile access/modifications
...
-- Create trigger on profiles table for UPDATE and DELETE
CREATE TRIGGER audit_profile_changes
AFTER UPDATE OR DELETE ON public.profiles
FOR EACH ROW
EXECUTE FUNCTION public.audit_profile_access();

-- ===== CLEANUP OLD AUDIT LOGS (optional - keep 90 days) =====
