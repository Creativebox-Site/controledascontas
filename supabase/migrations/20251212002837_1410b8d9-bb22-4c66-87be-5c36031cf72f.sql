-- Fix create_default_categories to validate that caller owns the user_id
-- This prevents privilege escalation where users could create categories for other users
...
BEGIN
  -- ===== SECURITY VALIDATION =====
  -- Validate that the caller owns the user_id to prevent privilege escalation
