...
export const WeeklySummary = ({
  ...
  const loadWeeklyData = async () => {
    ...
    const {
      data: transactions
    } = await supabase.from("transactions").select("*, categories(name, color)").eq("user_id", userId).eq("currency", currency).gte("date", format(weekStart, "yyyy-MM-dd")).lte("date", format(weekEnd, "yyyy-MM-dd"));
    if (transactions) {
      const income = transactions.filter(t => t.type === "income").reduce((sum, t) => sum + Number(t.amount), 0);
      const expenses = transactions.filter(t => t.type === "expense").reduce((sum, t) => sum + Number(t.amount), 0);

      // Encontrar categoria com maior gasto
      const categoryTotals = transactions.filter(t => t.type === "expense" && t.categories).reduce((acc, t) => {
      ...
      const topCategoryEntry = Object.entries(categoryTotals as Record<string, {
        amount: number;
        color: string;
  ...
  const savingsRate = weeklyData.income > 0 ? weeklyData.balance / weeklyData.income * 100 : 0;
  const getTip = () => {
  ...
  return <Card>
      <CardHeader>
        ...
        <CardDescription>
          {format(startOfWeek(new Date(), {
          locale: ptBR
        }), "dd MMM", {
          locale: ptBR
        })} -{" "}
          {format(endOfWeek(new Date(), {
          locale: ptBR
        }), "dd MMM", {
          locale: ptBR
        })}
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <p className={`text-2xl font-bold ${weeklyData.balance >= 0 ? "text-success" : "text-destructive"}`}>
        ...
        {/* Categoria Principal */}
        {weeklyData.topCategory && <div className="p-4 rounded-lg border bg-card">
            ...
            <div className="flex items-center justify-between">
              ...
              backgroundColor: weeklyData.topCategory.color
            }} />
