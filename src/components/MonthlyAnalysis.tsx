...
export const MonthlyAnalysis = ({
  ...
  const loadMonthlyData = async () => {
    ...
    } = await supabase.from("transactions").select("*, categories(name, color)").eq("user_id", userId).eq("currency", currency).gte("date", format(currentStart, "yyyy-MM-dd")).lte("date", format(currentEnd, "yyyy-MM-dd"));

    // Buscar transações do mês passado
    const {
      data: lastTransactions
    } = await supabase.from("transactions").select("*, categories(name, color)").eq("user_id", userId).eq("currency", currency).gte("date", format(lastStart, "yyyy-MM-dd")).lte("date", format(lastEnd, "yyyy-mm-dd"));
    if (currentTransactions && lastTransactions) {
      ...
      const currentExpenses = currentTransactions.filter(t => t.type === "expense").reduce((sum, t) => sum + Number(t.amount), 0);
      const lastIncome = lastTransactions.filter(t => t.type === "income").reduce((sum, t) => sum + Number(t.amount), 0);
      const lastExpenses = lastTransactions.filter(t => t.type === "expense").reduce((sum, t) => sum + Number(t.amount), 0);

      // Calcular mudanças por categoria
      const currentCategories = currentTransactions.filter(t => t.type === "expense" && t.categories).reduce((acc, t) => {
      ...
      const lastCategories = lastTransactions.filter(t => t.type === "expense" && t.categories).reduce((acc, t) => {
        const name = (t.categories as any)?.name || "Sem categoria";
        if (!acc[name]) acc[name] = 0;
        acc[name] += Number(t.amount);
        return acc;
      }, {} as Record<string, number>);
      const categoryChanges = Object.entries(currentCategories as Record<string, {
        amount: number;
        color: string;
      }>).map(([name, {
        amount,
        ...
        const percentChange = lastAmount > 0 ? (amount - lastAmount) / lastAmount * 100 : amount > 0 ? 100 : 0;
        return {
          name,
          current: amount,
          last: lastAmount,
      ...
      .sort((a, b) => Math.abs(b.percentChange) - Math.abs(a.percentChange)).slice(0, 3);
      setData({
        currentIncome,
        currentExpenses,
        lastIncome,
