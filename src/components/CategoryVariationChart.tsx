...
export const CategoryVariationChart = ({
  userId,
  currency
}: CategoryVariationChartProps) => {
  ...
  const loadCategoryData = async () => {
    ...
    } = await supabase.from("transactions").select("*, categories(name, color)").eq("user_id", userId).eq("currency", currency).eq("type", "expense").gte("date", format(currentStart, "yyyy-MM-dd")).lte("date", format(currentEnd, "yyyy-MM-dd"));
    const {
      data: lastTransactions
    } = await supabase.from("transactions").select("*, categories(name, color)").eq("user_id", userId).eq("currency", currency).eq("type", "expense").gte("date", format(lastStart, "yyyy-MM-dd")).lte("date", format(lastEnd, "yyyy-MM-dd"));
    if (currentTransactions && lastTransactions) {
      const currentCategories = currentTransactions.filter(t => t.categories).reduce((acc, t) => {
      ...
      const lastCategories = lastTransactions.filter(t => t.categories).reduce((acc, t) => {
        const name = (t.categories as any)?.name || "Sem categoria";
        if (!acc[name]) acc[name] = 0;
        acc[name] += Number(t.amount);
        return acc;
      }, {} as Record<string, number>);
      const categoryData: CategoryData[] = Object.entries(currentCategories as Record<string, {
        amount: number;
        color: string;
      }>).map(([name, {
        amount,
      ...
      }).sort((a, b) => b.current - a.current).slice(0, 8); // Top 8 categorias
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {data.map((category, index) => <div key={index} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-semibold ${category.variation > 0 ? "text-destructive" : category.variation < 0 ? "text-success" : "text-muted-foreground"}`}>
